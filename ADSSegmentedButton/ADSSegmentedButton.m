 ...
 *
 */
- (instancetype)initWithTitles:(NSArray<NSString *> *)arrayTitles tags:(NSArray<NSNumber *> *)arrayTags minimumButtonWidth:(CGFloat)minimumButtonWidth alignment:(ADSSegmentedButtonAlignment)alignment {
    if ([self init]) {
        [self resetSegmentedButtonsWithTitles:arrayTitles tags:arrayTags minimumButtonWidth:minimumButtonWidth alignment:alignment];
    }
    return self;
}
...
- (void)resetSegmentedButtonsWithTitles:(NSArray<NSString *> *)arrayTitles tags:(nullable NSArray<NSNumber *> *)arrayTags minimumButtonWidth:(CGFloat)minimumButtonWidth alignment:(ADSSegmentedButtonAlignment)alignment {
    ...
    [arrayTitles enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [btn mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            if (alignment == ADSSegmentedButtonAlignmentJustified) {
            ...
            else if (alignment == ADSSegmentedButtonAlignmentLeft) {
                if (minimumButtonWidth) {
                    make.width.mas_equalTo(minimumButtonWidth);
                }
                else {
                    make.width.equalTo(self).multipliedBy(1.0 / btnCount).priorityMedium();
                }
 ...
 */
- (void)configBottomLineWithHighlightLineColor:(UIColor *)highlightLineColor highlightLineHeight:(CGFloat)highlightLineHeight backgroundLineColor:(UIColor *)backgroundLineColor backgroundLineHeight:(CGFloat)backgroundLineHeight lineWidth:(CGFloat)lineWidth {
...
- (void)selectedButtonWithTag:(NSInteger)tag {
    ...
    !self.normalFont ?: [self.selectedButton.titleLabel setFont:self.normalFont];
    !self.selectedFont ?: [currentBtn.titleLabel setFont:self.selectedFont];
    
    [self.scrollView scrollRectToVisible:currentBtn.frame animated:YES];
    if (!self.selectedButton || self.selectedButton.tag != currentBtn.tag) {
...
- (void)p_configButtonAppearance {
    [self.buttonArray enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        !self.normalFont ?: [obj.titleLabel setFont:self.normalFont];
    }];
}
#pragma mark - Override

- (void)updateConstraints {
    if (_moveLine && self.selectedButton) {
